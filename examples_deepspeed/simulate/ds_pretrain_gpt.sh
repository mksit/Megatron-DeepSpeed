...
prescale_grad="true"
jobname="gpt_n${num_layers}_hs${hidden_size}_seq${seq_len}"
if [[ $moe_num_experts -gt 1 ]]; then
    jobname="${jobname}_e${moe_num_experts}"
fi
jobname="${jobname}_gb${global_batch_size}_mb${batch_size}_g${num_gpus}"
...
log_file="${log_path}/${jobname}_${current_time}.log"
...
megatron_options=" \
    ...
    --expert-interval ${moe_expert_interval} \
    --moe-loss-coeff ${mlc} \
...
config_json="ds_config_gpt_gbs${global_batch_size}_mbs${batch_size}_log${log_interval}_zero${zero_stage}.json"
...
run_cmd="deepspeed --include=localhost:0 --master_port 1234 ${dir}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} > $log_file 2>&1"
echo ${run_cmd}
eval ${run_cmd}
set +x
